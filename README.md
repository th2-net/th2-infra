...
ingress:
  host: example.com
```
* add custom annotations if required into `ingress.annotations.extra` (in key: value format). These annotations will be added to all ingress-rules generated by th2-infra

### Define custom annotations
Add custom annotations into `commonAnnotations` (in key: value format). These annotations will be added to all deployments, configMaps and ingresses that are created by th2-infra.
